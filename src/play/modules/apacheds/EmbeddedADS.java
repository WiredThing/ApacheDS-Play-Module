...
public class EmbeddedADS {
    ...
    private void initDirectoryService(File workDir) throws Exception {
        ...
        Partition rioPartition = addPartition("rio", "dc=riotinto,dc=org");

        // Index some attributes on the apache partition
        addIndex(rioPartition, "objectClass", "ou", "uid");

        // And start the service
        service.startup();

        // Inject the rio root entry
        if (!service.getAdminSession().exists(rioPartition.getSuffixDn())) {
            DN dnRio = new DN("dc=RioTinto,dc=Org");
            ServerEntry entryRio = service.newEntry(dnRio);
            entryRio.add("objectClass", "top", "domain", "extensibleObject");
            entryRio.add("dc", "Rio");
            service.getAdminSession().add(entryRio);
        }
